...
def BC_scan(
    barcodes, start_scan, end_scan, seq
):  ## Given start and end position of scanning sequence, look for barcodes
    str_list = [
        seq[i : i + 16] for i in range(start_scan, end_scan)
    ]  ## barcode file has "\n" in encoding
    bc_intersect = list(set(str_list).intersection(barcodes))
    if bc_intersect:
        bc_intersect = [
            element for element in bc_intersect
...
def TDetectRev(
    rev_seq,
):  ## If it was actually the reverse strand then it looks at T9 in the reverse complement
    try:
...
def TSODetectRev(
    revEnd_seq,
):  ## If it was actually the reverse strand then it looks at TSO in the reverse complement
...
def prelim(args):
    ...
    bc_file = args.bcClust.replace("\xa0", "")
...
def addToDict(d, line, rn):
    ...
    if fwd_ix == rev_ix == -1:  ## If valuError, output is -1, so no polyT found
            ...
            if "no_bc" in bc_found.get("bc_intersect"):
                ...
                d["Cluster"].append(
                    [
                        ...
                        for x in [
                            barcodes.index(item)
                            for item in bc_found.get("bc_intersect")
                        ]
                    ]
                )
        ...
        fwd_ix != -1 and rev_ix == -1
    ):  ## PolyT found in sequence but NOT the reverse complement
        d["T9_status"].append("poly_T_found")
        ...
        if (
            bc_found_f
            and bc_found_r
            and bc_found_r.get("bc_intersect") != "no_bc"
            and bc_found_f.get("bc_intersect") != "no_bc"
        ):
            ...
            bc_found_f and not bc_found_r
        ):  ## Barcode found in fwd strand, reverse strand polyT was a false positive
            d["T9_status"].append("poly_T_found")
            d["position"].append([fwd_ix])
            d["Strand_info"].append("fwd")
            ...
            if "no_bc" in bc_found.get("bc_intersect"):
                ...
                d["Cluster"].append(
                    [
                        ...
                        for x in [
                            barcodes.index(item)
                            for item in bc_found_f.get("bc_intersect")
                        ]
                    ]
                )
                UMI_start = int(bc_found.get("bc_pos")[0]) + 16
            ...
            bc_found_r and not bc_found_f
        ):  ## Barcode found in reverse strand, fwd T9 was a false positive
            d["T9_status"].append("poly_T_found")
            d["position"].append([rev_ix])
            d["Strand_info"].append("rev")
            d["BarcodeFound"].append(bc_found_f.get("bc_intersect"))
...
def writeTooShort(args, tooShort, childNo):
    ...
    if childNo == 1:
        ...
        tmp_df.to_csv(
            "%stoo_short%d" % (args.outDir, childNo),
            sep="\t",
            index=False,
            header=False,
        )
    ...
    os.system(
        'cat %stoo_short%d | tr -d "[ ]\'" >> %sTooShort.csv'
        % (args.outDir, childNo, args.outDir)
    )
...
def parse_args():
    ...
    parser.add_argument(
        "bcClust", type=str, help="single cell barcode-cluster assignment"
    )
    parser.add_argument(
        "--outDir", default="./", type=str, help="directory to put output in"
    )
    parser.add_argument(
        "--numProc", default=10, type=int, help="number of simultaneous processes"
    )
    parser.add_argument(
        "--chemistry", default="v2", type=str, help="10x chemistry version - v2 or v3"
    )
